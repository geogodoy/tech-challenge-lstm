# 📊 PROGRESS - Tech Challenge LSTM

> Acompanhamento do projeto de previsão de ações com LSTM
> **Fase 4 - Pós-graduação em Machine Learning Engineering**

---

## 🗺️ Mapa Geral do Projeto

| Etapa | Status | Descrição | Documentação |
|-------|--------|-----------|--------------|
| 1 | ✅ | Configuração do Ambiente | [Ver detalhes](./docs/etapas/ETAPA_01_Setup_Ambiente.md) |
| 2 | ✅ | Coleta de Dados (yfinance) | [Ver detalhes](./docs/etapas/ETAPA_02_Coleta_Dados.md) |
| 3 | ✅ | Pré-processamento | [Ver detalhes](./docs/etapas/ETAPA_03_Preprocessamento.md) |
| 4 | ✅ | Modelo LSTM | [Ver detalhes](./docs/etapas/ETAPA_04_Modelo_LSTM.md) |
| 5 | ✅ | Treinamento | [Ver detalhes](./docs/etapas/ETAPA_05_Treinamento.md) |
| 6 | ✅ | Avaliação (MAE, RMSE, MAPE) | Concluída |
| 7 | ✅ | Salvamento do Modelo | Concluída |
| 8 | ✅ | API FastAPI | Concluída |
| 9 | ✅ | Docker e Deploy | Concluída |

**Progresso:** `██████████████████████████████` **100%** (9/9 etapas) 🏆

---

## 📁 Estrutura do Projeto

```
tech-challenge-lstm/
├── 📄 README.md               # Descrição do projeto
├── 📄 PROGRESS.md             # Este arquivo
├── 📄 requirements.txt        # Dependências Python
├── 📄 .gitignore              # Arquivos ignorados
├── 🐳 Dockerfile              # Containerização
│
├── 📁 src/                    # Código-fonte
│   ├── data_collection.py     # ✅ Etapa 2
│   ├── preprocessing.py       # ✅ Etapa 3
│   ├── model.py               # ✅ Etapa 4
│   ├── train.py               # ✅ Etapa 5
│   ├── evaluate.py            # ✅ Etapa 6
│   └── app.py                 # ⏳ Etapa 8
│
├── 📁 models/                 # Artefatos salvos
│   ├── model_lstm.pth         # ✅ Modelo treinado
│   ├── scaler.pkl             # ✅ Normalizador
│   ├── config.pkl             # ✅ Configurações
│   └── training_history.png   # ✅ Gráfico de treino
│
├── 📁 data/                   # Dados coletados
│   └── data_PETR4_SA.csv      # ✅ Dados históricos
│
├── 📁 docs/                   # Documentação
│   └── etapas/                # Documentos por etapa
│       ├── README.md          # Índice
│       ├── ETAPA_01_*.md      # ✅
│       ├── ETAPA_02_*.md      # ✅
│       ├── ETAPA_03_*.md      # ✅
│       ├── ETAPA_04_*.md      # ✅
│       └── ETAPA_05_*.md      # ✅
│
├── 📁 notebooks/              # Jupyter notebooks
│
└── 📁 docs - fase 4 /         # Material de referência
    ├── etapa 1 - redes neurais/
    ├── etapa 2 - NLP/
    ├── etapa 3 - comp. vision/
    ├── etapa 4 - gen. IA/
    └── tech challenge/
```

---

## ✅ Etapas Concluídas

### Etapa 1: Configuração do Ambiente
- [x] Estrutura de pastas criada
- [x] Ambiente virtual (venv) configurado
- [x] Dependências instaladas (requirements.txt)
- [x] .gitignore configurado
- [x] Dockerfile base criado

### Etapa 2: Coleta de Dados
- [x] yfinance funcionando
- [x] Ticker PETR4.SA selecionado
- [x] Período 2018-2024 (6 anos)
- [x] CSV salvo em `data/`

### Etapa 3: Pré-processamento
- [x] Normalização (MinMaxScaler 0-1)
- [x] Janelas deslizantes (60 dias)
- [x] Split treino/teste (80/20)
- [x] Conversão para tensores PyTorch
- [x] Scaler e config salvos

### Etapa 4: Modelo LSTM
- [x] Classe StockLSTM criada
- [x] Arquitetura: LSTM → Dropout → Linear
- [x] 31,051 parâmetros treináveis
- [x] Teste com dados sintéticos passou

### Etapa 5: Treinamento
- [x] MSELoss configurado
- [x] Adam optimizer (lr=0.001)
- [x] 100 épocas executadas
- [x] Train Loss: 0.002085
- [x] Val Loss: 0.003514
- [x] Modelo salvo em `models/model_lstm.pth`
- [x] Gráfico de histórico salvo

### Etapa 6: Avaliação
- [x] Carregar modelo treinado
- [x] Fazer previsões no conjunto de teste (286 amostras)
- [x] Calcular métricas: MSE, RMSE, MAE, MAPE
- [x] Reverter normalização (scaler.inverse_transform)
- [x] Plotar previsões vs valores reais
- [x] Diagnóstico: **BOM** (MAPE = 6.74%)

### Etapa 7: Salvamento
- [x] Artefatos salvos em `models/`
- [x] model_lstm.pth (130 KB)
- [x] scaler.pkl, config.pkl
- [x] training_history.png
- [x] predictions_vs_actual.png

---

### Etapa 8: API FastAPI
- [x] Endpoint POST /predict
- [x] Endpoint GET /health
- [x] Validação de entrada (Pydantic)
- [x] Carregar modelo na inicialização (lifespan)
- [x] Schemas de entrada/saída documentados
- [x] Documentação automática (Swagger/OpenAPI)

---

## ✅ Etapa 9: Docker e Deploy (CONCLUÍDA)

- [x] Dockerfile completo com health check
- [x] docker-compose.yml criado
- [x] Build da imagem Docker (`stock-predictor-api`)
- [x] Container executando na porta 8000
- [x] Endpoint `/health` testado e funcionando
- [x] Endpoint `/predict` testado e funcionando

---

## 📚 Referências

- **Guia do Tech Challenge:** `docs - fase 4 /tech challenge/Guia de Predição de Ações com LSTM`
- **Aula 03 - Arquiteturas:** `docs - fase 4 /etapa 1/Aula 03 - Arquiteturas de Redes Neurais Profundas.txt`
- **Documentação Detalhada:** `docs/etapas/`

---

## 📊 Métricas do Treinamento (Modelo Otimizado)

| Métrica | Valor |
|---------|-------|
| Épocas | 100 |
| Tempo total | ~30.8s |
| Train Loss final | 0.000693 |
| Val Loss final | 0.001367 |
| Melhor Val Loss | 0.001190 (época 68) |
| Dispositivo | CPU |
| **Hidden Size** | **100** (otimizado) |

---

## 📊 Métricas de Avaliação (Modelo Otimizado)

| Métrica | Valor | Interpretação |
|---------|-------|---------------|
| MSE | 0.7964 | Erro quadrático médio |
| **RMSE** | **R$ 0,89** | Erro médio em reais |
| MAE | R$ 0,70 | Erro absoluto médio |
| **MAPE** | **3,83%** | Erro percentual médio |

### Diagnóstico Final
- **Status:** ✅ EXCELENTE
- O modelo tem precisão excelente (MAPE < 5%)
- Em média, erra apenas R$ 0,89 por previsão
- Pronto para uso em produção

### Comparação: Antes vs Depois da Otimização

| Métrica | Antes | Depois | Melhoria |
|---------|-------|--------|----------|
| MAPE | 6,74% | **3,83%** | **43% melhor** |
| RMSE | R$ 1,43 | **R$ 0,89** | **38% melhor** |
| MAE | R$ 1,15 | **R$ 0,70** | **39% melhor** |

---

---

## 🐳 Docker - Comandos e Testes

### Build da Imagem
```bash
docker build -t stock-predictor-api .
```

### Executar Container
```bash
docker run -d --name stock-api -p 8000:8000 stock-predictor-api
```

### Testar Health Check
```bash
curl http://localhost:8000/health
# Resposta: {"status":"healthy","model_loaded":true,"device":"cpu","ticker":"PETR4.SA","seq_length":60}
```

### Testar Previsão
```bash
curl -X POST http://localhost:8000/predict \
  -H "Content-Type: application/json" \
  -d '{"prices": [36.5, 36.8, ... (60 valores)]}'
# Resposta: {"predicted_price":38.03,"currency":"BRL","ticker":"PETR4.SA",...}
```

### Parar e Remover Container
```bash
docker stop stock-api && docker rm stock-api
```

---

*Última atualização: 2026-02-19 (Etapa 9 concluída - PROJETO FINALIZADO 🏆)*
